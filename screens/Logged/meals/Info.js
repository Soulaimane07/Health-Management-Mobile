    ...
    <BottomSheetModalProvider>
        <View style={styles.container}>
            ...
            <ScrollView vertical style={{paddingTop: 10, paddingHorizontal: 20,}}>
                {data?.filter((val)=>{
                    searchData == "" && ""
                    if(val.name.toLowerCase().includes(searchData.toLowerCase())){ return val}
                }).map((val,key)=>(
                    <TouchableOpacity onPress={()=> OpenModal() & setSelected(val._id)} style={[styles.item, key+1 == data?.length && {marginBottom: "60%"}]} key={key}>
                        <View style={{alignItems: 'center', flexDirection: 'row'}}>
                            <Image source={{uri: `${ServerLink}/${val.image}`}} style={[{marginRight: 20, width: 40, height: 40}]} />
                            <Text style={{fontSize: 16}}> {val.name} </Text>
                        </View>
                        {mealData?.find(element => element.name == val.name) ? <Aicon name="checkcircle" color="#3FC495" size={24} /> : <Aicon name="pluscircleo" size={20} />}
                    </TouchableOpacity>
                ))}
            </ScrollView>
        ...
        >
            <View style={styles.modalView}>
                <SheetBody meal={route.params.meal} selectedId={selected} CloseModal={CloseModal} />
            </View>
        </BottomSheetModal>
    </BottomSheetModalProvider>
